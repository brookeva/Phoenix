...
allTractsPHX.group <-
  rbind(
    st_centroid(allTractsPHX)[buffer,] %>%
      st_drop_geometry() %>%
      left_join(allTractsPHX) %>%
      st_sf() %>%
      mutate(TOD = "TOD"),
    st_centroid(allTractsPHX)[buffer, op = st_disjoint] %>%
      st_drop_geometry() %>%
      left_join(allTractsPHX) %>%
      st_sf() %>%
      mutate(TOD = "Non-TOD")) %>%
...
```{r}
allTractsPHX <- rbind(tracts2020PHX,tracts2016PHX)

allTractsPHX.rings <-
  st_join(st_centroid(dplyr::select(allTractsPHX, GEOID, year)), 
          multipleRingBuffer(st_union(PHXstops), 47520, 2640)) %>%
  st_drop_geometry() %>%
  left_join(dplyr::select(allTractsPHX, GEOID, MedRent, year), 
            by=c("GEOID"="GEOID", "year"="year")) %>%
  st_sf() %>%
  mutate(distance = distance / 5280) #convert to miles

  ggplot() +
    geom_sf(data=multipleRingBuffer(st_union(PHXstops), 47520, 2640)) +
